...
const Register = () => {
    ...
    useEffect(() => {
        ...
        if (err === 'oauth_not_configured') {
            setError('التسجيل عبر Google/GitHub غير مفعّل — يحتاج إعداد بيانات OAuth في الإعدادات');
            return;
        }
        if (err === 'oauth_no_email') {
            setError('لم نتمكن من الحصول على بريدك الإلكتروني من مزود OAuth — استخدم التسجيل بالبريد');
            return;
        }
        if (err === 'email_exists') {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-indigo-900 to-indigo-700 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
                ...
                {oauthLoading ? (
                    <p className="text-center text-gray-500 py-8">جاري تحميل بيانات OAuth...</p>
                ) : (
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                type="email"
                                className={`input-field ${oauthMode ? 'bg-gray-50 text-gray-600' : ''}`}
                                value={form.email}
                        ...
                        <div>
                            <label className="label">
                                كلمة السر
                                {oauthMode && <span className="text-gray-400 font-normal mr-1">(اختياري)</span>}
                            </label>
                            <input
                                type="password"
                                className="input-field"
