...
const createWill = async (req, res) => {
    try {
        ...
        const existing = await pool.query(
            'SELECT id FROM wills WHERE user_id = $1', [req.user.id]
        );
        if (existing.rows.length > 0) {
            return res.status(409).json({
                success: false,
                message: 'لديك وصية بالفعل. يمكنك تعديلها فقط'
            });
        }

        const result = await pool.query(
            `INSERT INTO wills (user_id, title, description, checkin_interval_days, grace_period_days)
             VALUES ($1, $2, $3, $4, $5) RETURNING *`,
            [req.user.id, title, description || null, checkin_interval_days || 30, grace_period_days || 7]
        );
